...
//    static Hero hero = new Hero();
    ...
    public void heroMovement(Hero hero,Enemies enemy, Map map, RandomPositionGenerator random) {
        ...
        if (checkMovement(input, map)) {
            ...
            randomItemSearch(keyValue, enemy, hero);
        } else {
            JOptionPane.showMessageDialog(null, "Valor incorreto, digite o correto");
            heroMovement(hero, enemy, map, random);
        }

    }

    public void randomItemSearch(String keyValue, Enemies enemy, Hero hero) {

        if (keyValue.equals(random.getEnemies())) {
            random.randomTypeOfEnemy(enemy);
